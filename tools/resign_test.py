    ...
    def generate_new_key(self, exist_keys):
        ...
        for record in self.zone_records:
            if record.rrtype == "NSEC3":
                command.append("-3")
                if self.algorithm in NOT_NSEC3_ALGORITHMS:
                    logger.warning(("algorithm %d of key %s is not compatible with" +
                                    "NSEC3. Using algorithm 7 instead"),
                                   self.algorithm, self.tag)
                    self.algorithm = 7
                break
