...
ipaddr_re = compile('^((\d{1,3}\.){3}\d{1,3})|(([\dA-Fa-f]{1,4})|(:)|(:[\dA-Fa-f]{1,4})){2,7}[\dA-Fa-f]$')
...
class Generator:
    ...
    def write_deckard_set(self):

        unb_set = open(self.unbound_path, "w+")
        set_string = self.generate_set()
        unb_set.write(set_string)
        unb_set.close()

        knot_file = open(self.kresd_path, "w+")
        knot = self.generate_kresd()
            ...
            if item.name == 'network':
                ...
                if 'recursion-transport' in item:
                    ...
                    if 'l2-protocols' in item['recursion-transport']:
                        ...
                        if 'ivp4' in protocols:
                            if 'ivp6' in protocols:
                                server += str("\tdo-ip6 = yes\n\tdo-ip4 = yes\n")
                            else:
                                server += str("\tdo-ip6 = no\n\tdo-ip4 = yes\n")
                        else:
            ...
            if item.name == 'resolver':
                ...
                if 'stub-zones' in item:
                    ...
                    for sz in item['stub-zones']:
                        ...
                        if ipaddr_re.match(str(sz["nameserver"])):
                            stub += str("\tstub-addr: \"{}\"\n".format(temp))
                        else:
                            stub += str("\tstub-host: \"{}\"\n".format(temp))
    ...
    def generate_set(self):
        unb_conf = self.generate_unbound()
        mock = self.load_mock()

        set_string = unb_conf + mock
        return set_string
